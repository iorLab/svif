...
1. **Complete and verify the `v0.2.0-preview.1` target-main candidate.** Keep implementation on short-lived branch `release/svif-v0.2.0-preview.1`, run repository integrity, portable contracts, the full runtime/unit suite, Skill validation, and confirm the candidate preserves Skills-only single-source packaging and Agnir `v0.1.1` compatibility.
2. **Push one immutable candidate and require green CI.** Record the exact candidate SHA. The release branch is staging for authoritative `main`, not a separate durable Project truth.
3. **Validate the one-line install intent on both supported Preview surfaces.** Use the immutable candidate SHA in Codex CLI and ChatGPT desktop/Codex. Start with an ordinary non-Agnir Project; do not pre-initialize Agnir or pre-create `SVIF.yaml`. Verify first-use classification, non-destructive bootstrap, continuation of the original task, verification, checkpoint, and fresh-context resume. Then repeat against an initialized Project to prove idempotent normal resume.
4. **Integrate and publish only after both client baselines pass.** Recheck `origin/main`; reconcile/retest if it advanced. Otherwise advance `main` to the verified candidate, create immutable tag `v0.2.0-preview.1`, publish a GitHub Prerelease, and run a tag-based installation smoke. Do not move a failed tag; repair into `v0.2.0-preview.2`.
5. **Record post-release evidence and restore main-only governance.** Checkpoint the exact tag, release id/URL, CI, supported-client installation observations, Agnir activation/checkpoint results, and any unverified provenance. Preserve the release branch tip SHA; delete the temporary branch only after explicit cleanup confirmation.
6. **Continue the separate public/personal ChatGPT path.** Resolve or formally clarify the publisher-verification gate, then submit the exact tested Skills-only package to the universal Plugins Directory. Record scan/review evidence, explicitly Publish after approval, and validate the first individual-user ChatGPT Web installation without conflating it with the Repository Preview.
7. **Repair only observed friction, then expand neutrality/surface evidence.** Keep `plugin/skills/svif/SKILL.md` single-sourced; add no MCP merely for publication. Add broader non-repository and multi-project evidence without making GitHub, Cloudflare, ChatGPT, Cursor, or another execution environment universal dependencies.
