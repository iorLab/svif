...
5. An operational Agnir package upgrade and a Core/profile compatibility migration are different operations. Neither is implied by installing or invoking Svif.
...
Agnir Agent activation and Core discovery are distinct layers. Validate the canonical instruction surface selected by the installed Agnir contract, not just heading/link presence. When README is the canonical instruction surface, validate the full required Project-operation instructions there. When `AGNIR.md` is canonical, validate the full procedure in `AGNIR.md` and require the README Agnir heading to remain only the compatible locator specified by the selected release.
...
`AGENTS.md` remains locator-only. Preserve unrelated existing instructions and never silently override a material conflict. The selected canonical activation surface—legacy README procedure or current `AGNIR.md` procedure—owns the full Agnir Project instructions. Equivalent locators are idempotent; material conflicts must be surfaced to the Principal.
